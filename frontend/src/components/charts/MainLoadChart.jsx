...
export default function MainLoadChart({ data, unmanagedPeak, managedPeak }) {
  // Use real peak values when available, fall back to confirmed simulation values
  const uPeak = unmanagedPeak ?? 4100
  const mPeak  = managedPeak  ?? 1509
  ...
  return (
    <div className="glass-card relative h-[460px] p-5">
      ...
      {/* Dynamic peak annotations */}
      <motion.div
        className="pointer-events-none absolute left-[68%] top-[102px] text-xs font-semibold text-[#e74c3c]"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 2.5 }}
      >
        {uPeak.toLocaleString('en-IN')} kW ⚠
      </motion.div>
      <motion.div
        className="pointer-events-none absolute left-[42%] top-[244px] text-xs font-semibold text-[#b49cff]"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 2.7 }}
      >
        {mPeak.toLocaleString('en-IN')} kW ✓
      </motion.div>
    </div>
  )
}
  ...
  return (
    <div className="rounded-lg border border-[var(--border-primary)] bg-[#1e2d40] p-3 text-xs shadow-xl">
      ...
      <div className="space-y-1 text-[var(--text-secondary)]">
        <div>With GridPilot: <span className="text-[var(--color-managed)] font-bold">{row.managed?.toLocaleString('en-IN')} kW</span></div>
        <div>Without GridPilot: <span className="text-[var(--color-unmanaged)]">{row.unmanaged?.toLocaleString('en-IN')} kW</span></div>
        <div>Solar: <span className="text-[#00d4aa]">{row.solar ?? 0} kW</span></div>
        <div>Status: <span className={row.status === 'CRITICAL' ? 'text-[var(--color-dirty)]' : 'text-[var(--color-success)]'}>{row.status ?? 'STABLE'}</span></div>
        {row.transformer_pct !== undefined && (
          <div>Transformer: <span className="text-[var(--text-primary)]">{row.transformer_pct?.toFixed(1)}%</span></div>
        )}
      </div>
    </div>
  )
