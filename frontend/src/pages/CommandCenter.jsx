  ...
  return (
    <div className="relative">
      <Particles
        ...
        options={{
          particles: { number: { value: 36 }, color: { value: '#7c5cbf' }, opacity: { value: 0.15 }, size: { value: { min: 1, max: 2 } }, move: { enable: true, speed: 0.25 } },
...
function Timeline() {
  const events = [
    ...
    ['02:00', 'Carbon CLEAN - 2,000 kW ramp', '#27ae60'],
    ['05:00', 'Clean window closing', '#f9ca24'],
    ['06:30', '500/500 vehicles at 80% SoC', '#4ecdc4'],
    ['07:00', 'Dispatch ready - ₹33K saved', '#27ae60'],
  ]
  return (
    <div className="relative mt-20 h-80">
      <motion.div className="absolute left-8 right-8 top-1/2 h-[2px] origin-left bg-gradient-to-r from-[#2d3f55] to-[#7c5cbf]" initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 2.5, ease: 'easeOut' }} />
      {events.map(([time, label, color], index) => (
        <motion.div key={time} className="absolute top-1/2" style={{ left: `${8 + index * 17}%` }} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3 + index * 0.25 }}>
          <div className="h-3 w-3 rounded-full" style={{ background: color, boxShadow: index === 2 ? `0 0 0 8px ${color}33` : undefined }} />
