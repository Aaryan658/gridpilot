...
Maps optimizer indices (0-39) to physical charger IDs (CP001-CP040)
and vehicle IDs (V001-V040) using the Vahan CY2025 fleet composition.
"""

from database.models import SessionLocal, VehicleChargerMap
import uuid

# Vahan CY2025 fleet composition — exact proportions for 40 vehicles
# (same % mix as the original 600-vehicle fleet, rounded to sum to 40)
FLEET_COMPOSITION = [
    {"model": "Tata Nexon EV",  "count": 11, "battery_kwh": 30.2, "charger_kw": 7.4},
    {"model": "Tata Tiago EV",  "count":  6, "battery_kwh": 19.2, "charger_kw": 3.3},
    {"model": "MG Windsor EV",  "count":  7, "battery_kwh": 38.0, "charger_kw": 7.4},
    {"model": "Mahindra BE6",   "count":  7, "battery_kwh": 59.0, "charger_kw": 7.2},
    {"model": "Tata Curvv EV",  "count":  6, "battery_kwh": 55.0, "charger_kw": 7.2},
    {"model": "MG ZS EV",       "count":  3, "battery_kwh": 50.3, "charger_kw": 7.4},
]


def seed_vehicle_charger_map(depot_id: str) -> bool:
    """
    Creates 40 VehicleChargerMap records for the given depot_id.
    Does nothing if records already exist for this depot.
    Returns True if seeding occurred, False if already seeded.
    """
    expected_count = sum(spec["count"] for spec in FLEET_COMPOSITION)
    db = SessionLocal()
    try:
        ...
        if existing > 0:
            print(
                f"[SEED] Stale vehicle mapping for {depot_id} ({existing} records, "
                f"expected {expected_count}) — re-seeding."
            )
            db.query(VehicleChargerMap).filter(
                VehicleChargerMap.depot_id == depot_id
            ).delete()
            db.commit()

        print(f"[SEED] Creating {expected_count} vehicle-charger mappings for {depot_id}...")
        index = 0
...
def get_full_mapping(depot_id: str) -> list[dict]:
    """Returns all 40 records as list of dicts for the given depot."""
    db = SessionLocal()
    try:
        records = (
